...
from src.news.schema import NewsSummaryRequestSchema, NewsSummaryCustomModelRequestSchema, PromptRequest
...
def test_news_summary_with_custom_model(mocker, test_token):
    ...
    request_body = NewsSummaryCustomModelRequestSchema(content="Test news content", llm_model="openai")
    ...
    request_body = NewsSummaryCustomModelRequestSchema(content="Test news content", llm_model="anthropic")
    response = client.post("/api/v1/news/news_summary_custom_model", json=request_body.dict(), headers=headers)
    assert response.status_code == 200
    json_response = response.json()
